...
import { isArray, isObject, isString } from "@zayne-labs/toolkit-type-helpers";
...
const getHTTPErrorMessages = (errorData: BaseApiErrorResponse) => {
	...
	return detailMessages.length > 0 ? detailMessages : [message];
};
		...
		hooks: {
			onError: (ctx: ErrorContext<{ ErrorData: BaseApiErrorResponse }>) => {
				...
				if (isHTTPError(ctx.error)) {
					getHTTPErrorMessages(ctx.error.errorData).forEach((message) => toast.error(message));
					return;
				}
